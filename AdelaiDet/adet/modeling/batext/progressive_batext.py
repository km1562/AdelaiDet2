...
class Progressive_BAText(nn.Module):
    """
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.fcos_head = Progressive_FCOSHead(cfg, [input_shape[f] for f in self.in_features])
...
class Progressive_FCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        head_configs = {"cls": (cfg.MODEL.FCOS.NUM_CLS_CONVS,
                        ...
                        "share": (cfg.MODEL.FCOS.NUM_SHARE_CONVS,
                                  cfg.MODEL.FCOS.USE_DEFORMABLE)
                        # "progressive": (cfg.MODEL.FCOS.NUM_BOX_CONVS,
                        #           cfg.MODEL.FCOS.USE_DEFORMABLE)
                        }
    ...
    def decode(self, top_feats, progressive):
        N, C, H, W = top_feats.shape
        # TO_REMOVE = 1
        progressive = progressive.reshape(N, C, H * W)
        top_feats = top_feats.reshape(N, C, H * W)

        top_feats[:, 0, :] = top_feats[:, 0, :] + W * progressive[:, 0, :]
        top_feats[:, 1, :] = top_feats[:, 1, :] + H * progressive[:, 1, :]

        top_feats[:, 2, :] = top_feats[:, 2, :] + W * progressive[:, 2, :]
        top_feats[:, 3, :] = top_feats[:, 3, :] + H * progressive[:, 3, :]

        top_feats[:, 4, :] = top_feats[:, 4, :] + W * progressive[:, 4, :]
        top_feats[:, 5, :] = top_feats[:, 5, :] + H * progressive[:, 5, :]

        top_feats[:, 6, :] = top_feats[:, 6, :] + W * progressive[:, 6, :]
        top_feats[:, 7, :] = top_feats[:, 7, :] + H * progressive[:, 7, :]

        top_feats[:, 8, :] = top_feats[:, 8, :] + W * progressive[:, 8, :]
        top_feats[:, 9, :] = top_feats[:, 9, :] + H * progressive[:, 9, :]

        top_feats[:, 10, :] = top_feats[:, 10, :] + W * progressive[:, 10, :]
        top_feats[:, 11, :] = top_feats[:, 11, :] + H * progressive[:, 11, :]

        top_feats[:, 12, :] = top_feats[:, 12, :] + W * progressive[:, 12, :]
        top_feats[:, 13, :] = top_feats[:, 13, :] + H * progressive[:, 13, :]

        top_feats[:, 14, :] = top_feats[:, 14, :] + W * progressive[:, 14, :]
        top_feats[:, 15, :] = top_feats[:, 15, :] + H * progressive[:, 15, :]
    ...
    def forward(self, x, top_module=None, yield_bbox_towers=False):
        ...
        for l, feature in enumerate(x):
            ...
            if top_module is not None:
                temp_progressive = self.progressive(bbox_tower)
                temP_top_feats = top_module(bbox_tower)
                top_feats.append(self.decode(temP_top_feats, temp_progressive))
