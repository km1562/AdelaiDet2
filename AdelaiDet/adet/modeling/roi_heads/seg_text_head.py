...
from .multi_path_fuse_module import build_mutil_path_fuse_module
...
class MaskHead(nn.Module):
    ...
    def forward(self, features):  #整个思路，就是conda，然后一系列卷积回来不是吗
...
class Seg_Text_Head(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        """
        super(Seg_Text_Head, self).__init__()
        # fmt: off
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.coordconv:
            ...
            for i in range(len(features)):
                ...
                all_feat = mask_feat + features[i] #这里是累加特征的，确实
                mask_features.append(all_feat)
            features = mask_features

        if self.training:
            # image_shape = [tar.image_size for tar in targets]
            # image_shape = [targets[0].image_size[1], targets[0].image_size[0]]
            image_shape = [[target[0].image_size[1], target[0].image_size[0]] for target in targets]
            beziers = [p.beziers for p in targets]
            if not self.aet:
                # targets = torch.cat([x.text for x in targets], dim=0)
                targets_text = torch.cat([x.text for x in targets], dim=0)
            else:
                ...
                for k in range(len(targets)):
                    ...
                    targets_text[k] = torch.cat([targets[k].text, targets[k].text[rec_assign]], dim=0)
                # targets = torch.cat([x for x in targets], dim=0)
                targets_text = torch.cat([x for x in targets_text], dim=0)
                cat_beziers = []
                for ix in range(len(beziers)):
                    cat_beziers.append(cat((beziers[ix], beziers2[ix]), dim=0))
                beziers = cat_beziers

                # TODO 得到globbal_context,1、配置文件 2、
        ...
        #TODO 融合特征,global_context, 修改他

        if self.mutil_path_fuse_on:
            bezier_features = self.mutil_path_fuse_module(global_context, bezier_features)
        ...
        if self.training:
            ...
            preds, rec_loss = self.recognizer(bezier_features, targets_text)
            rec_loss *= 0.05
            # losses = {'rec_loss': rec_loss}
            losses.update({"rec_loss" : rec_loss})
            return None, losses
        else:
